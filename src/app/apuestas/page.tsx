...
function ApuestasPartido({
  ...
  const rows = useMemo(() => participantes.map(p => {
    const pred = p.pronosticosGrupos?.[partidoId];
    return { nombre: p.nombre, pred };
      ...
      <button onClick={() => setOpen(o => !o)} className="w-full text-left p-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-gray-400 text-sm ml-2 shrink-0">
            {open ? '▲' : '▼'} {conApuesta}/{participantes.length}
          </span>
        </div>
      </button>
            ...
            <tbody className="divide-y divide-white/5">
              {rows.map(({ nombre, pred }) => (
                <tr key={nombre} className="hover:bg-white/5">
                  ...
                  <td className="px-3 py-2.5 text-center font-black text-white text-lg">
                    {pred !== undefined ? pred.golesLocal : <span className="text-gray-600">—</span>}
                  </td>
                  <td className="px-3 py-2.5 text-center font-black text-white text-lg">
                    {pred !== undefined ? pred.golesVisitante : <span className="text-gray-600">—</span>}
                  </td>
                  {resultado && (
                    <td className="px-3 py-2.5 text-center">
                      {pred !== undefined
                        ? <PtsChip p={pts(resultado.golesLocal, resultado.golesVisitante, pred.golesLocal, pred.golesVisitante)} />
